<div class="w-full space-y-2">
    ...
    <div class="relative custom-select-wrapper">
        ...
        <button type="button" class="custom-select-trigger relative w-full bg-white border border-black/20 rounded-md shadow-sm py-[17px] px-5 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
            <span class="block truncate text-black-100/50 text-base font-normal leading-6 custom-select-text" data-placeholder="{{ $placeholder ?? 'Select an option' }}">{{ $placeholder ?? 'Select an option' }}</span>
        ...
        <ul class="custom-select-options absolute z-10 mt-3 w-full bg-white shadow-lg max-h-60 rounded-[10px] py-1 overflow-auto focus:outline-none hidden" tabindex="-1" role="listbox">
            ...
            <li class="custom-pointer select-none cursor-pointer relative py-2 pl-3 pr-9 hover:bg-black-100/10 transition-all duration-300" data-value="{{ $option['value'] }}">
                <span class="text-black-100 text-base leading-6 block truncate">{{ $option['label'] }}</span>
            </li>
            @endforeach
        </ul>
    </div>
