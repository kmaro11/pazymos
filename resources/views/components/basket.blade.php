<div class="max-w-[420px] w-full bg-gray-400 rounded-[30px] pt-5 px-[14px] md:px-[30px] pb-7 md:pb-[30px]" data-component="basket">
    ...
    @if ($description)
    <p class="text-black-100 font-['Helvetica_Neue'] text-sm font-normal leading-[22px] mb-[30px] md:mb-6">
        {{ $description }}
    </p>
    @endif
    ...
    <div class="space-y-2.5 mb-6">
        ...
        <div class="flex items-center bg-white gap-x-2 pl-[15px] pr-2.5 py-[7px] rounded-[100px]">
            ...
            <div>
                <p class="text-black-100 font-['Helvetica_Neue'] text-sm font-normal leading-5">{{ $item['date'] }}</p>
                <p class="text-black-100/60 font-['Helvetica_Neue'] text-[11px] font-medium leading-5">{{ $item['time'] }}</p>
            </div>
            @if ($item['available'] > 0)
            <span class="ml-auto bg-blue text-black-100 text-center font-['Helvetica_Neue'] text-xs font-medium leading-6 tracking-[0.6px] uppercase max-w-[97px] w-full rounded-[100px] block">
                {{ $item['available'] }} / {{ $item['total'] }} VIETŲ
            </span>
            @else
            <span class="ml-auto bg-black-100/20 text-white text-center font-['Helvetica_Neue'] text-xs font-medium leading-6 tracking-[0.6px] uppercase max-w-[97px] w-full rounded-[100px] block">
                {{ $item['available'] }} / {{ $item['total'] }} VIETŲ
            </span>
            @endif
        </div>
    ...
    <div class="flex justify-between items-center mb-3">
        <div class="text-black-100 font-['Helvetica_Neue'] text-base font-medium leading-6">Kaina:</div>
        <div class="flex items-center gap-x-1 text-black-100 font-['Helvetica_Neue'] text-2xl font-medium leading-8">
            <div>€</div>
